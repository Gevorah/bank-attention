...
public class Bank {
	...
	public String showPriorityQueue() {
		if(priorityqueue.size()==0) return "empty queue";
		else return priorityqueue.print();
	}
	...
	private static Client[] merge(Client[] lr, Client[] l, Client[] r, int lf, int rg) { //3n+2
		...
		while(i<lf&&j<rg) {
			...
			d1 = LocalDateTime.of(Integer.parseInt(date1[2]), Integer.parseInt(date1[1]), Integer.parseInt(date1[0]), 0, 0);//1
			d2 = LocalDateTime.of(Integer.parseInt(date2[2]), Integer.parseInt(date2[1]), Integer.parseInt(date2[0]), 0, 0);//1
			if(d1.compareTo(d2)<=0) lr[k++] = l[i++];
			else lr[k++] = r[j++];
		}
		while(i < lf) lr[k++] = l[i++];
	    while(j < rg) lr[k++] = r[j++];
	...
	public void sortByCC() {
		...
		for(Client tmp:clientsToSort) a.insert(tmp);
		a.minHeap();
		clientsToSort.clear();
		int size = a.size();
